...
class LogGraphPanel(wx.Panel):
    ...
    def __init__(self, parent, sysComps):
        ...
        # Number of point staying in memory (buffer)
        self.sizeBuffer = 5 * 60 * 4
        # Number of elements buffered since last write
        self.nData2log = 0
        ...
        # self.slider_zoom = wx.Slider(
        #     self, -1, value=1, minValue=1,
        ...
        #           self.on_slider_width, self.slider_zoom)
        ...
        # self.controllersHBox.Add(
        #     self.slider_label, 0, wx.ALIGN_CENTER_VERTICAL)
        # self.controllersHBox.Add(
        #     self.slider_zoom, 0,
        #     wx.SHAPED | wx.FIXED_MINSIZE | wx.ALIGN_CENTER_VERTICAL)
        ...
        for comp in self.sysComps:
            ...
            for var in comp.vars.values():
                ...
                if isinstance(var, SysVarAnalog):
                    ...
                    # else:
                    # split this variable to two unipolar variables
                    # for the sake of plotting
                    #     self.plottedAnalogData[
                    #         name + '_POS'] = AnalogData([0, var.range[1]])
                    #     self.plottedAnalogData[
                    #         name + '_NEG'] = AnalogData([var.range[0], 0])

                    #     self.plottedLines[name + '_POS'] = self.axes.plot(
                    #         self.time, self.plottedAnalogData[
                    #             name + '_POS'].data,
                    #         '-', lw=lineWidth, color=color,
                    #         label=nameOnLegend,
        ...
        self.lineLegendDictReverseDict = {
            v: k for k, v in self.lineLegendDict.items()}
    ...
    def AddData(self, name, value):
        ...
        # if not self.hasBipolarRange(name):
        normVal = LogGraphPanel.NormalizePositiveValue(
            value, self.all[name].yRange)
        self.all2plot[name].extend(np.array(normVal, 'f'))
        # else:
        #     normPosVal = self.NormalizePositiveValue(
        #         value, self.plottedAnalogData[name + '_POS'].yRange)
        #     normNegVal = self.NormalizePositiveValue(
        #         value, self.plottedAnalogData[name + '_NEG'].yRange)
        #     self.plottedAnalogData[
        #         name + '_POS'].data += [normPosVal if value >= 0 else None]
        #     self.plottedAnalogData[
        #         name + '_NEG'].data += [normNegVal if value < 0 else None]
    ...
    def FinishUpdate(self):
        ...
        if (cfg.signalsLogFile is None or
                cfg.timeCurrentSignalsLogFile.day != self.time.data[-1].day or
                self.time.data[-1] - cfg.timeCurrentSignalsLogFile >=
                timedelta(minutes=self.logFileLength)):
    ...
    def WriteDataInLog(self, _n):
        ...
        for i in range(0, _n):
            cfg.signalsLogFile.write(
                str(self.time.get(_n)[i].strftime('%H:%M:%S.%f')) +
    ...
    def Redraw(self):  # , firstTime=False):
        ...
        if self.nData2plot > 2:
            ...
            if not self.cb_freeze.IsChecked():
                ...
                #     lower=self.time[max(0, len(self.time) -
                #                         int(self.slider_zoom.GetValue() *
                #                             60 * cfg.app.updateFrequency))],
                #     upper=self.time[-1])

                self.axes.set_xbound(
                    lower=self.time.data[-self.nData2plot],
                    upper=self.time.data[-1])

                # Update data to plot
                for name in self.analog.keys():
                    self.plottedLines[name].set_xdata(
                        self.time.get(self.nData2plot))
                    self.plottedLines[name].set_ydata(
                        self.all2plot[name].get(self.nData2plot))
